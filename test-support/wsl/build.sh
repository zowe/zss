...
if ls "$DEPS_PREBUILT"/*.o >/dev/null 2>&1; then
  ...
  # The two version macros must reach the compiler as STRING LITERALS. They are
  # held in their own variables and expanded QUOTED, unlike the rest of the
  # flags: an unquoted expansion re-splits the word and the escaped quotes
  # arrive as literal backslashes, leaving YAML_VERSION_STRING as an int (api.c
  # then returns it as a const char *) and CONFIG_VERSION as a bare token that
  # breaks quickjs.c's string concatenation with "expected ')'".
  YAML_DEFS="-DYAML_VERSION_MAJOR=0 -DYAML_VERSION_MINOR=2 -DYAML_VERSION_PATCH=5 \
             -DYAML_DECLARE_STATIC=1"
  YAML_VERSION_DEF='-DYAML_VERSION_STRING="0.2.5"'
  QJS_DEFS="-DCONFIG_BIGNUM=1 -D_GNU_SOURCE=1"
  QJS_VERSION_DEF='-DCONFIG_VERSION="2021-03-27"'
  for y in api reader scanner parser loader writer emitter dumper; do
    $CLANG -c $CFLAGS $YAML_DEFS "$YAML_VERSION_DEF" -I "$DEPS/libyaml/include" \
  ...
  for q in cutils quickjs quickjs-libc libunicode libbf libregexp; do
    $CLANG -c $CFLAGS $QJS_DEFS "$QJS_VERSION_DEF" -I "$DEPS/quickjs" \
