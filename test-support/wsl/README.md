> **The common-c side now lives on a branch, not in this patch.**
> `common-c-wsl-overlay.patch` was captured against a zowe-common-c tree 305
> commits behind staging and no longer applies. Use the `dev/wsl-build` branch
> of zowe-common-c instead, which is that overlay rebased onto current staging
> (minus the charsets.c/h part, which merged upstream as #630). The patch file
> is kept only as the historical record of what was ported.
